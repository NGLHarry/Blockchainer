...
// 返回交易的数组
func (bc *BlockChain) FindUnspentTransations(address string, txs []*Transaction) []Transaction {
	fmt.Println(address)
	// 输出未花费输出的交易
...
//查找可用的未消费的输出信息
func (bc *BlockChain) FindSpendableOutputs(address string, amout int, txs []*Transaction) (int, map[string][]int) {
	//{"11111":[1,2,3],"00000":[2,3,5]}
	// 字典，存储交易id，Vout里面的未花费TXOutput的index
	unspentOutpus := make(map[string][]int)
	// 查看未花费
	unspentTXs := bc.FindUnspentTransations(address, txs)
...
//创建一个带有创世区块节点的区块链
func CreateGenenisBlockchain(genesisAddress string) {
